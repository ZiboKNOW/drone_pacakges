...
class features_extractor(nn.Module):
    def __init__(self, pretrained, down_ratio, feat_shape, in_channels=None):
        ...
        pretrain_dict = torch.load('/home/cmic2/ros_workspace/src/drone_detection/model/model_80.pth')
    ...
    def forward(self, images, trans_mats, shift_mats, map_scale):
        with torch.no_grad():
            ...
            single_x = global_x
            # single_x[0] = self.relu(single_x[0])
...
class decoder(nn.Module):
    ...
    def forward(self, global_x, round_id):
        ...
        with torch.no_grad():
            ...
            z = {}
            for head in self.heads:
                global_z[head] = self.__getattr__(head)(y[-1])
            ##################################################
